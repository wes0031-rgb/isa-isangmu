...
export default function ChecklistScreen() {
  ...
  const [contract, setContract] = useState<ContractType>('월세');
    ...
    useCallback(() => {
      (async () => {
        ...
        if (saved) {
          ...
          setConcerns((req.special_concerns ?? []).join(', '));
          setMode('result');
  ...
  async function handleShare() {
    ...
    const lines = [
      `📋 MoveWise 이사 체크리스트`,
      `이사일: ${moveDate} · ${region} · ${household} · ${contract}`,
      '',
  ...
  async function handleNew() {
    Alert.alert('새 체크리스트', '저장된 체크리스트를 지우고 새로 만드시겠어요?', [
      ...
      {hasChildren && (
        <Section label="자녀 학교급">
          <SegmentGroup
            ...
            onChange={(v: any) => setSchoolLevel(v)}
          />
        </Section>
      )}

      <Section label="특이 상황 (쉼표 구분)">
        <TextInput
          value={concerns}
          onChangeText={setConcerns}
          placeholder="예: 보증금 반환 우려, 월세 인상"
          style={styles.input}
        />
      </Section>

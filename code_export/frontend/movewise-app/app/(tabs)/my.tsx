...
export default function MyScreen() {
  ...
  async function handleSaveApi() {
    ...
    try {
      ...
      setHealth(h);
      setHealthError(null);
      Alert.alert('저장 완료', `${h.service} v${h.version} 연결 확인됨`);
    } catch (e: any) {
  ...
  async function handleSaveName() {
    ...
    Alert.alert('저장됨', `이름: ${userName || '(빈 값)'}`);
  }
...
const styles = StyleSheet.create({
  ...
  statusText: { ...typography.caption, fontSize: 11, flex: 1 },
  infoRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.md,
  ...
  infoLabel: { ...typography.caption, fontWeight: '600', flex: 1 },
  infoValue: { ...typography.caption, fontSize: 11, color: colors.textSub },
  dangerBtn: {
    flexDirection: 'row',
    alignItems: 'center',

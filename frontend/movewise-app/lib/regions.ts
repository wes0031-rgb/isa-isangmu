/**
 * 한국 행정구역 데이터 — 17 시/도 + 시/군/구.
 * 사용자가 시/도를 먼저 선택하면 해당 하위 시/군/구 목록이 나타난다.
 */
...
export const REGIONS: Region[] = [
  ...
  },
  {
    ...
    districts: [
      ...
      '안양시 만안구', '안양시 동안구',
      '안산시 상록구', '안산시 단원구',
      '부천시', '광명시', '평택시', '동두천시', '안성시', '의정부시',
      '시흥시', '군포시', '의왕시', '하남시', '오산시', '이천시',
...
export function formatRegion(sido: string, district: string): string {
  return `${sido} ${district}`;
}

export function parseRegion(full: string): { sido: string; district: string } | null {
  for (const r of REGIONS) {
    if (full.startsWith(r.sido)) {
      const rest = full.slice(r.sido.length).trim();
      if (r.districts.includes(rest)) {
        return { sido: r.sido, district: rest };
      }
    }
  }
  return null;
